...
class PasswordReset {
    ...
    public function resetPassword($password) {
        ...
        if (!$email) {
            return "No reset email found!";
        }

        $hashed_password = md5($password);

        if (!$this->updatePassword($email, $hashed_password, 'users')) {
            return "Failed to update password in users table!";
        }

        $user_table = $this->determineUserTable($email);
        if ($user_table && !$this->updatePassword($email, $hashed_password, $user_table)) {
            return "Failed to update password in $user_table table!";
        }
    ...
    private function determineUserTable($email) {
        $tables = ['hr', 'employee', 'manager'];
    ...
    private function updatePassword($email, $hashed_password, $table) {
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    if (isset($_POST['password']) && isset($_POST['confirm_password'])) {
        ...
        if ($password !== $confirm_password) {
            ...
            $error_message = $passwordReset->resetPassword($password);
        }
    }
}
?>
